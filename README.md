...
Telegram-бот + async Python-обёртка над приватным upstream generation API (`app.phygital.plus`) для
скриптовой генерации изображений на платной подписке. Публичного API нет — клиент собран по recon
HAR-капчам.
...
аплоадит каждый файл в upstream storage **один раз**, кэширует `file_obj_id` + `sha256` в
...
workflows/
  ...
  brand_docs.py           docs/*.md → upstream storage (upload + sha256-кэш в storage/brand_docs.json)
...
> **Авторизация — только через recon.** У upstream-сервиса нет публичного API и нет login-by-password
...
- **Global inflight=5**: `global_sem` берётся уже внутри `_execute_task` под submit/poll к upstream API, чтобы запуск пары пользователей не «съел» весь глобальный бюджет до того, как кто-то третий получит хотя бы одно место.
...
- `client/api.py:_request` пишет `DEBUG | METHOD URL` на каждый HTTP-вызов к upstream API.
...
Stdlib-only, без зависимостей. Корреляции «запрос ↔ конкретный сценарий» нет — логи upstream-вызовов
...
Прямые вызовы upstream API через `PhygitalClient` (harness-level, без Telegram). Использует
